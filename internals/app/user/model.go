...
type User struct {
	...
	DateOfBirth string `json:"date_of_birth" validate:"omitempty,dateformat"`
	Gender      string `json:"gender"`
}
type UserProfileDetails struct {
	Username    string `json:"username" validate:"omitempty,min=8,max=24"`
	FirstName   string `json:"firstname" validate:"omitempty,min=4,max=10"`
	LastName    string `json:"lastname" validate:"omitempty,min=4,max=10"`
	PhoneNumber string `json:"phone" validate:"omitempty,len=10"`
	Email       string `json:"email" validate:"omitempty,email"`
	DateOfBirth string `json:"dateofbirth" validate:"omitempty,dateformat"`
